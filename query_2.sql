...
SELECT 
    ...
    CASE 
        WHEN m.FECHA_EMP_RECEPCION IS NULL THEN NULL
        WHEN m.FECHA_EMP_RECEPCION = '1900-01-01' THEN NULL
        WHEN DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_RECEPCION) > 30 THEN 30
        WHEN DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_RECEPCION) < -30 THEN -30
        ELSE DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_RECEPCION)
    END AS DIAS_RECEPCION_A_EMPLEADOR,
    
    CASE 
        WHEN m.FECHA_EMP_ENVIO IS NULL THEN NULL
        WHEN m.FECHA_EMP_ENVIO = '1900-01-01' THEN NULL
        WHEN DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_ENVIO) > 30 THEN 30
        WHEN DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_ENVIO) < -30 THEN -30
        ELSE DATEDIFF(day, m.FECHA_RECEPCION, m.FECHA_EMP_ENVIO)
    END AS DIAS_RECEPCION_A_ENVIO,
    
    CASE 
        WHEN DATEDIFF(day, m.FECHA_INICIO, m.FECHA_RECEPCION) > 30 THEN 30
        WHEN DATEDIFF(day, m.FECHA_INICIO, m.FECHA_RECEPCION) < -30 THEN -30
        ELSE DATEDIFF(day, m.FECHA_INICIO, m.FECHA_RECEPCION)
    END AS DIAS_INICIO_A_RECEPCION,
    
    CASE 
        WHEN DATEDIFF(day, m.FECHA_INICIO, m.FECHA_EMISION_DT) > 30 THEN 30
        WHEN DATEDIFF(day, m.FECHA_INICIO, m.FECHA_EMISION_DT) < -30 THEN -30
        ELSE DATEDIFF(day, m.FECHA_INICIO, m.FECHA_EMISION_DT)
    END AS DIAS_INICIO_A_EMISION,
    
    CASE 
        WHEN DATEDIFF(day, m.FECHA_EMISION_DT, m.FECHA_RECEPCION) > 30 THEN 30
        WHEN DATEDIFF(day, m.FECHA_EMISION_DT, m.FECHA_RECEPCION) < -30 THEN -30
        ELSE DATEDIFF(day, m.FECHA_EMISION_DT, m.FECHA_RECEPCION)
    ...
    CASE 
        ...
        WHEN DATEDIFF(day, m.FECHA_EMP_RECEPCION, m.FECHA_EMP_ENVIO) > 3 
        THEN 1 
        ELSE 0 
    ...
    CASE 
        WHEN TRY_CAST(SPLIT_PART(m.PRESTADOR_RUT, '-', 1) AS INTEGER) BETWEEN 48000000 AND 49999999 THEN 1
        ELSE 0
    ...
    DATEDIFF(month, m.FECHA_SUSCR_CONTRATO, m.FECHA_RECEPCION) AS MESES_DESDE_CONTRATO,
    DATEDIFF(month, m.FECHA_INGRESO_PREST, m.FECHA_RECEPCION) AS MESES_ANTIGUEDAD_PRESTADOR,
    m.COMUNA_REPOSO,
    m.COMUNA_EMISION,
    m.EMPLEADOR_RUT,
    ...
    IFF(m.FECHA_EMISION_DT IS NOT NULL, 1, 0) AS TARGET_REQUIERE_REVISION,
