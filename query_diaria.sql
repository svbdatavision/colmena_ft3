...
cpa_historico_prestador AS (
    ...
    WHERE TO_DATE(FECHA_RECEPCION) < DATEADD(day, -1, CURRENT_DATE())  -- Solo registros anteriores a ayer
...
AND TO_DATE(ALFIL.FECHA_RECEPCION) = DATEADD(day, -1, CURRENT_DATE());
...
USING (
    WITH licencias_ayer AS (
        ...
        WHERE DATE_TRUNC('DAY', FECHA_RECEPCION) = DATEADD(DAY, -1, CURRENT_DATE())
    ),
